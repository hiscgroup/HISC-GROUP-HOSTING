        ...
        Citizen.Wait(0)
            ...
            for i = 1, #shisha do
                ...
                if dist < 3.5 then
					...
					if not buyad then
						ESX.Game.Utils.DrawText3D(vector3(shishaPos.x, shishaPos.y, shishaPos.z+1), "Press [~y~E~w~] to start a session. (20$)", 0.7)
					end
						...
						Citizen.CreateThread(function()
							while true do
								if sessionStarted == true then
									ESX.Game.Utils.DrawText3D(vector3(shishaPos.x, shishaPos.y, shishaPos.z+0.4), "~c~Press ~b~\"H\"~c~ to pull the tube.", 0.7)
									ESX.Game.Utils.DrawText3D(loc, "~c~Press ~b~\"F\"~c~ to end the session.", 0.7)
								end
								Citizen.Wait(0)
							end
						end)
